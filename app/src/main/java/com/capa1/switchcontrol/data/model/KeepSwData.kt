...
class KeepSwData @Inject constructor (
    ...
    fun initOperation() {
        mqttManager.connect()
        _swList = something.sortedBy { it.row } //getStoredData()
    ...
    override fun notifyNewMessage(topic: String, msg: String) {
        ...
        if(id == allSwId) {
            ...
            _swList = newFlashData.swList.sortedBy { it.row }
            coroutineScope.launch { swList.emit(_swList) }
            //saveData()
            initializeSwList()
        }
        else {
           Log.i(TAG,"LLEGÓ UN ID INESPERADO: $id")
        }
    }
    ...
    override fun notifyMqttState(mqttState: MqttState) {
        ...
        Log.i(Global.TAG, "....MQTT STATE: $mqttState")
    }
    ...
    fun imageClick(id: String) {
        Log.i(TAG, " SI SI llega el click del id: $id")
        for (data in _swList) {
            if (data.id == id) {
